...
static struct text_range text_ranges[] = {
	...
	{ "__sram_text_start", "__sram_text_end" },
};
#define text_range_text     (&text_ranges[0])
#define text_range_inittext (&text_ranges[1])
#define text_range_sramtext (&text_ranges[4])
...
static int symbol_valid(struct sym_entry *s)
{
	...
	if (!all_symbols) {
		...
		if ((s->addr == text_range_text->end &&
			...
			(s->addr == text_range_sramtext->end &&
				strcmp((char *)s->sym + offset, text_range_sramtext->etext)))
			return 0;
	}
...
static void write_src(void)
{
	...
	for (i = 0; i < table_cnt; i++) {
		if (toupper(table[i].sym[0]) != 'A') {
			char uncompressed_sym[256];
			expand_symbol(table[i].sym, table[i].len, uncompressed_sym);
			if (_text <= table[i].addr)
				printf("\tPTR\t_text + %#llx\t; %s\n",
					table[i].addr - _text, uncompressed_sym);
			else
				printf("\tPTR\t_text - %#llx\t; %s\n",
					_text - table[i].addr, uncompressed_sym);
		} else {
			printf("\tPTR\t%#llx\n", table[i].addr);
		}
	}
