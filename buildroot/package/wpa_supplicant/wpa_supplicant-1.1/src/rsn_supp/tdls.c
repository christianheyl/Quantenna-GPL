...
#define TPK_LIFETIME 86400 /* 24 hours */
#define TPK_RETRY_COUNT 3
#define TPK_TIMEOUT 2500 /* in milliseconds */
...
struct wpa_tdls_peer {
	...
	u8 dtoken;
	int cs_interval;	/* channel switch interval */
...
static inline int wpa_tdls_over_qhop_enabled(struct wpa_sm *sm)
{
	return (sm->tdls_over_qhop_en &&
				(sm->ap_role != IEEE80211_EXTENDER_ROLE_NONE));
}

static int wpa_tdls_ext_bssid_allowed(struct wpa_sm *sm, u8 *bssid)
{
	int bssid_allowed = 0;
	int i;

	if (!is_zero_ether_addr(sm->mbs_bssid) &&
			!os_memcmp(sm->mbs_bssid, bssid, ETH_ALEN)) {
		bssid_allowed = 1;
	} else {
		for (i = 0; i < WPA_QTN_MAX_RBS_NUM; i++) {
			if (!is_zero_ether_addr(sm->rbs_bssid[i]) &&
					!os_memcmp(sm->rbs_bssid[i], bssid, ETH_ALEN)) {
				bssid_allowed = 1;
				break;
			}
		}
...
static int wpa_tdls_bssid_check(struct wpa_sm *sm, struct wpa_tdls_lnkid *lnkid)
{
	int bssid_mismatched;
	if (wpa_tdls_over_qhop_enabled(sm))
		bssid_mismatched = !wpa_tdls_ext_bssid_allowed(sm, lnkid->bssid);
	else
		bssid_mismatched = os_memcmp(sm->bssid, lnkid->bssid, ETH_ALEN);

	return bssid_mismatched;
}

static inline int
wpa_tdls_check_ext_bssid_ie(const u8 *ie)
{
#define	WPA_QTN_OUI		0x862600	/* Quantenna OUI */
#define WPA_QTN_OUI_EXT_BSSID	0x15	/* Extender BSSID */
	return (ie[1] > 3 &&
		((WPA_GET_LE32(ie + 2) & 0x00ffffff) == WPA_QTN_OUI) &&
			(ie[5] == WPA_QTN_OUI_EXT_BSSID));
}

void wpa_tdls_update_qhop_info(struct wpa_sm *sm, u8 role, u8 *ie)
{
	int i;
	struct wpa_qtn_ext_bssid *ext_ie;

	sm->ap_role = role;
	if (ie && wpa_tdls_check_ext_bssid_ie(ie)) {
		ext_ie = (struct wpa_qtn_ext_bssid *)ie;
		os_memcpy(sm->mbs_bssid, ext_ie->mbs_bssid, sizeof(sm->mbs_bssid));
		sm->rbs_num = ext_ie->rbs_num;
		os_memcpy(sm->rbs_bssid[0], ext_ie->rbs_bssid[0], sizeof(sm->rbs_bssid));
	} else {
	...
	wpa_printf(MSG_INFO, "TDLS: update qhop info, ap_role %d, mbs_bssid "
				MACSTR ", rbs_num %d, rbs_bssid:", sm->ap_role,
				MAC2STR(sm->mbs_bssid), sm->rbs_num);
	for (i = 0; i < WPA_QTN_MAX_RBS_NUM; i++)
		wpa_printf(MSG_INFO, MACSTR, MAC2STR(sm->rbs_bssid[i]));
}
...
static void wpa_tdls_tpk_retry_timeout(void *eloop_ctx, void *timeout_ctx)
{
	...
	if (peer->sm_tmr.count) {
		...
		wpa_tdls_do_teardown(sm, peer,
				WLAN_REASON_TDLS_TEARDOWN_UNREACHABLE, 1);
	}
}
...
static void wpa_tdls_tpk_timeout(void *eloop_ctx, void *timeout_ctx)
{
	 ...
	 * the direct link or trying to re-initiate it. Since driver will try to setup
	 * TDLS link periodically, just teardown the TDLS link simply here.
	 */
...
static void wpa_tdls_peer_free(struct wpa_sm *sm, struct wpa_tdls_peer *peer)
{
	...
	os_memset(peer->ext_bssid, 0, ETH_ALEN);
}
			    ...
			    struct wpa_tdls_lnkid *lnkid)
{
	...
	if (wpa_tdls_over_qhop_enabled(sm) &&
			!is_zero_ether_addr(peer->ext_bssid))
		os_memcpy(lnkid->bssid, peer->ext_bssid, ETH_ALEN);
	else
...
int wpa_tdls_send_teardown(struct wpa_sm *sm, const u8 *addr, u16 reason_code)
{
	...
	ielen = sizeof(lnkid);
	if (wpa_tdls_get_privacy(sm) && peer->tpk_set) {
...
skip_ies:
	...
	/* add link id IE */
	os_memcpy(pos, (u8 *)&lnkid, sizeof(lnkid));
	pos += sizeof(lnkid);
...
int wpa_tdls_switch_chan(void *eloop_ctx, void *timeout_ctx)
{
	struct wpa_sm *sm = eloop_ctx;
	struct wpa_tdls_peer *peer = timeout_ctx;

	if (peer == NULL) {
		wpa_printf(MSG_DEBUG, "TDLS: Null peer pointer");
		return -1;
	}

	if (!peer->tpk_success) {
		wpa_printf(MSG_DEBUG, "TDLS: Peer " MACSTR " not connected"
			" - cannot switch channel", MAC2STR(peer->addr));
		eloop_cancel_timeout(wpa_tdls_switch_chan, sm, peer);
		return -1;
	}

	wpa_sm_tdls_oper(sm, TDLS_SWITCH_CHAN, peer->addr);

	eloop_cancel_timeout(wpa_tdls_switch_chan, sm, peer);
	eloop_register_timeout(peer->cs_interval / 1000,
					(peer->cs_interval % 1000) * 1000,
					wpa_tdls_switch_chan, sm, peer);

	return 0;
}


int wpa_tdls_start_switch_chan(struct wpa_sm *sm, const u8 *addr, int interval)
{
	struct wpa_tdls_peer *peer;
	...
	if (peer == NULL) {
		wpa_printf(MSG_DEBUG, "TDLS: Could not find peer " MACSTR
		   " for channel switch", MAC2STR(addr));
		return -1;
	}
	...
	if (!peer->tpk_success) {
		wpa_printf(MSG_DEBUG, "TDLS: Peer " MACSTR
		   " not connected - cannot switch channel", MAC2STR(addr));
		return -1;
	}

	if (interval > 0) {
		peer->cs_interval = interval;
		eloop_register_timeout(interval / 1000, (interval % 1000) * 1000,
						wpa_tdls_switch_chan, sm, peer);
	}
...
skip_ies:
	/* add link id IE */
	os_memcpy(pos, (u8 *)lnkid, sizeof(struct wpa_tdls_lnkid));
	pos += sizeof(struct wpa_tdls_lnkid);
...
skip_rsnie:
	...
	wpa_hexdump(MSG_DEBUG, "TDLS: Initiator Nonce for DISCOVERY RESPONSE",
			peer->inonce, WPA_NONCE_LEN);
	os_memcpy(ftie->Snonce, peer->inonce, WPA_NONCE_LEN);

	wpa_hexdump(MSG_DEBUG, "TDLS: FTIE for DISCOVERY RESPONSE",
			(u8 *) ftie, sizeof(struct wpa_tdls_ftie));
...
skip_ies:
	...
	ret = wpa_tdls_tpk_send(sm, peer->addr, WLAN_TDLS_DISCOVERY_RESPONSE,
				 dialog_token, 0, rbuf, pos - rbuf);
	os_free(rbuf);
				   ...
				   const u8 *buf, size_t len)
{
	...
	lnkid = (struct wpa_tdls_lnkid *) kde.lnkid;
	if (wpa_tdls_bssid_check(sm, lnkid)) {
	...
	return wpa_tdls_send_discovery_response(sm, peer, lnkid, dialog_token);
}
				   ...
				   const u8 *buf, size_t len)
{
	...
	if (wpa_tdls_over_qhop_enabled(sm) &&
			os_memcmp(sm->bssid, lnkid->bssid, ETH_ALEN))
		os_memcpy(peer->ext_bssid, lnkid->bssid, ETH_ALEN);
...
skip_rsn:
	...
	wpa_tdls_generate_tpk(peer, sm->own_addr, lnkid->bssid);
...
skip_rsn:
	...
	if (wpa_tdls_send_tpk_m3(sm, src_addr, dtoken, lnkid, peer))
		goto error;
...
/**
 * wpa_tdls_pti - Initiate TDLS Peer Traffic Indicator exchange (send Traffic Indication frame)
 * @sm: Pointer to WPA state machine data from wpa_sm_init()
 * @peer: MAC address of the peer STA
 * Returns: 0 on success, or -1 on failure
 *
 */
int wpa_tdls_pti(struct wpa_sm *sm, const u8 *addr)
{
	struct wpa_tdls_peer *peer;
	struct wpa_tdls_lnkid *lnkid;
	u8 dialog_token;
	...
	if (peer == NULL) {
		wpa_printf(MSG_INFO, "TDLS: No matching entry found for "
			   "PTI " MACSTR, MAC2STR(addr));
		return 0;
	}

	dialog_token = peer->dtoken;

	wpa_printf(MSG_DEBUG, "TDLS: TDLS PTI for " MACSTR,
		   MAC2STR(addr));
	...
	/* 1) Link Identifier IE */
	lnkid = (struct wpa_tdls_lnkid*)pos;
	wpa_tdls_linkid(sm, peer, lnkid);
	pos += 2 + lnkid->ie_len;
	...
	/* request driver to send Teardown using this FTIE */
	ret = wpa_tdls_tpk_send(sm, addr,
				WLAN_TDLS_PEER_TRAFFIC_INDICATION, dialog_token, 0, rbuf,pos - rbuf);
	os_free(rbuf);

	return ret;
}

static int wpa_tdls_recv_pti_resp(struct wpa_sm *sm, const u8 *src_addr,
				  const u8 *buf, size_t len)
{
	struct wpa_tdls_peer *peer = NULL;
	...
	if (peer == NULL) {
		wpa_printf(MSG_INFO, "TDLS: No matching entry found for "
			   "PTI response " MACSTR, MAC2STR(src_addr));
		return 0;
	}

	wpa_tdls_tpk_retry_timeout_cancel(sm, peer, WLAN_TDLS_PEER_TRAFFIC_INDICATION);
				   ...
				   const u8 *buf, size_t len)
{
	...
	l2_packet_get_own_addr(sm->l2_tdls, l2_tdls_addr);
	if (os_memcmp(src_addr, l2_tdls_addr, ETH_ALEN) == 0) {
	...
	case WLAN_TDLS_PEER_TRAFFIC_RESPONSE:
		wpa_tdls_recv_pti_resp(sm, src_addr, buf, len);
		break;
	default:
...
int wpa_tdls_init(struct wpa_sm *sm)
{
	...
	if (linux_br_get(brname, sm->ifname) == 0) {
		wpa_printf(MSG_DEBUG, "Bridge interface %s configured for "
			   "TDLS action receive", brname);
		sm->l2_tdls = l2_packet_init(brname, sm->own_addr,
	...
	wpa_drv_tdls_oper(wpa_s, TDLS_DISABLE, NULL);
