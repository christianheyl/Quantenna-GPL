	     ...
	     int show_err)
{
	...
	if (len < IFNAMSIZ &&
	    ((op != IEEE80211_IOCTL_SET_APPIEBUF) && (op != IEEE80211_IOCTL_POSTEVENT) && (op != IEEE80211_IOCTL_FILTERFRAME))) {
		/*
		 * Argument data fits inline; put it there.
		 */
...
static int
get80211param(struct wpa_driver_madwifi_data *drv, int op, int *value)
{
	struct iwreq iwr;
	...
	if (ioctl(drv->sock, IEEE80211_IOCTL_GETPARAM, &iwr) < 0) {
		wpa_printf(MSG_DEBUG, "%s: Failed to get parameter (op %d)",
					__func__, op);
		return -1;
	}
			   ...
			   const u8 *key, size_t key_len)
{
	...
	case WPA_ALG_NONE:
		...
		if (key_idx == 4 || key_idx == 5) {
			/* Clear bip key here */
			os_memset(&drv->drv_igtk_wk, 0, sizeof(wk));
		}
		return 0;
	default:
...
static int check_bip(struct wpa_driver_madwifi_data *drv, const u8 *data, size_t len)
{
	const struct ieee80211_mgmt *mgmt;
	u16 fc, stype;
	const u8 *mmie;
	...
	if (check_mmie_mic(igtk->ik_keydata, data, len) < 0) {
...
void rx_mgmt(struct wpa_driver_madwifi_data *drv, const u8 *data, size_t len)
{
	const struct ieee80211_hdr *hdr;
	u16 fc, stype;
	...
	if ((hdr->addr1[0] & 0x01) &&
		...
		if (check_bip(drv, data, len) > 0)
			wpa_driver_madwifi_deauthenticate(drv, hdr->IEEE80211_SA_FROMDS, 2);

	}

}

static void madwifi_mgmt_receive(void *ctx, const u8 *src_addr, const u8 *buf,
				size_t len)
{
	struct wpa_driver_madwifi_data *drv = ctx;
	const struct ieee80211_mgmt *mgmt;
	u16 fc;

	/* Process deauth and disassoc for BIP */
	mgmt = (const struct ieee80211_mgmt *) buf;
...
static int madwifi_receive_mgmt_filter(struct wpa_driver_madwifi_data *drv)
{
	int ret = 0;
	struct ieee80211req_set_filter filt;
	...
	filt.app_filterype = (IEEE80211_FILTER_TYPE_DEAUTH | IEEE80211_FILTER_TYPE_DISASSOC);
			     ...
			     struct wpa_driver_associate_params *params)
{
	...
	if ((params->key_mgmt_suite == KEY_MGMT_802_1X_SHA256) ||
		...
		madwifi_receive_mgmt_filter(drv);
	}
	return ret;
}
...
static int wpa_driver_madwifi_get_capa(void *priv, struct wpa_driver_capa *capa)
{
	struct wpa_driver_madwifi_data *drv = priv;
	int mfp_options = NO_MGMT_FRAME_PROTECTION;
	...
	get80211param(drv, IEEE80211_PARAM_CONFIG_PMF, &mfp_options);
...
static int wpa_driver_madwifi_set_pairing_hash_ie(void *priv, const u8 *pairing_hash,
							size_t ies_len)
{
...
static int wpa_driver_madwifi_get_pairing_hash_ie(void *priv, u8 *pairing_hash,
							size_t ies_len, u8 *addr)
{
	...
	if (set80211priv(priv, IEEE80211_IOCTL_GETWPAIE, &ie, sizeof(ie), 1)) {
		wpa_printf(MSG_DEBUG, "%s: Failed to get WPA/RSN IE",
			   __func__);
		return -1;
	}
...
static void wpa_driver_madwifi_find_pid(void *priv)
{
	struct wpa_driver_madwifi_data *drv = priv;
	static const char *pid_file = "/tmp/hostapd.pid";
	static unsigned int max_loop = 100;
	int ret;
	char *buf;
	size_t len;
	unsigned int cnt = 0;
	...
	if (cnt == max_loop) {
		wpa_printf(MSG_DEBUG, "hostapd.pid not found\n");
		return;
	}

	buf = os_readfile(pid_file, &len);
	if (!buf) {
		wpa_printf(MSG_DEBUG, "read hostapd.pid failed\n");
		return;
	}

	drv->hostapd_pid = (pid_t)atoi(buf);
...
static void * wpa_driver_madwifi_init(void *ctx, const char *ifname)
{
	...
	if (set80211param(drv, IEEE80211_PARAM_WPA_STARTED, 1, 1) < 0) {
		wpa_printf(MSG_DEBUG, "%s: failed to set WPA state",
			   __FUNCTION__);
		goto fail3;
	}
...
static void wpa_driver_madwifi_deinit(void *priv)
{
	...
	if (set80211param(drv, IEEE80211_PARAM_WPA_STARTED, 0, 1) < 0) {
		wpa_printf(MSG_DEBUG, "%s: failed to clear WPA state",
			   __FUNCTION__);
	}
 ...
 * layers.
 */
static int wpa_driver_madwifi_mlme_setprotection(void *priv, const u8 *addr, int protect_type,
		                                  int key_type, const char *msg)
{
	...
	if (protect_type == MLME_SETPROTECTION_PROTECT_TYPE_RX_TX) {
		...
		return set80211priv(drv, IEEE80211_IOCTL_POSTEVENT, open_port_msg, os_strnlen(open_port_msg, MAX_WLAN_MSG_LEN), 1);
	}
	/* Messages for sending via iwevent */
	if (msg) {
		return set80211priv(drv, IEEE80211_IOCTL_POSTEVENT, msg, os_strnlen(msg, MAX_WLAN_MSG_LEN), 1);
	}
	return 0;
}

static int
wpa_driver_madwifi_set_fast_reassoc(void *priv, const int value, int *cur_value)
{
        struct wpa_driver_madwifi_data *drv = priv;
        int fast_reassoc = 0;

        wpa_printf(MSG_DEBUG, "%s: value=%d", __FUNCTION__, value);

	get80211param(drv, IEEE80211_PARAM_FAST_REASSOC, &fast_reassoc);
        if (cur_value) {
                *cur_value = fast_reassoc;
	}

        if (fast_reassoc != value) {
                return set80211param(drv, IEEE80211_PARAM_FAST_REASSOC, value, 1);
        }
...
static int wpa_driver_madwifi_send_tdls_mgmt(void *priv,
					...
					const u8 *buf, size_t len)
{
	...
	ret = set80211priv(priv, IEEE80211_IOCTL_SET_APPIEBUF, tdls_act,
			   sizeof(*tdls_act) + sizeof(*action_data) + len, 1);
...
static int wpa_driver_madwifi_tdls_oper(void *priv,
			enum tdls_oper oper, const u8 *peer)
{
	...
	if (ioctl(drv->sock, IEEE80211_IOCTL_EXT, &iwr) < 0) {
		wpa_printf(MSG_DEBUG, "%s: Failed to set tdls operation", __func__);
		return -1;
	}
...
const struct wpa_driver_ops wpa_driver_madwifi_ops = {
	...
	.get_capa		= wpa_driver_madwifi_get_capa,
	.set_operstate		= wpa_driver_madwifi_set_operstate,
	.mlme_setprotection	= wpa_driver_madwifi_mlme_setprotection,
	.set_pairing_hash_ie	= wpa_driver_madwifi_set_pairing_hash_ie,
	.get_pairing_hash_ie	= wpa_driver_madwifi_get_pairing_hash_ie,
        .set_fast_reassoc       = wpa_driver_madwifi_set_fast_reassoc,
#ifdef CONFIG_TDLS
	.send_tdls_mgmt		= wpa_driver_madwifi_send_tdls_mgmt,
	.tdls_oper		= wpa_driver_madwifi_tdls_oper,
#endif /* CONFIG_TDLS */
#endif /* HOSTAPD */
};
