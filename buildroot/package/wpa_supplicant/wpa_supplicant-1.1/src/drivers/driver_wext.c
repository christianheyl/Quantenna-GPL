...
static void
wpa_driver_wext_generate_wds_key(const u8 *mbs_addr, const u8 *rbs_addr,
		const u8 *pmk, u8 *wds_key)
{
	const u8 *addr[3];
	size_t len[3];
	...
	wpa_hexdump(MSG_DEBUG, "mbs_mac:", mbs_addr, ETH_ALEN);
	wpa_hexdump(MSG_DEBUG, "rbs_mac:", rbs_addr, ETH_ALEN);
	wpa_hexdump(MSG_DEBUG, "pmk:", pmk, PMK_LEN);
	wpa_hexdump(MSG_DEBUG, "wds_key:", wds_key, PMK_LEN);
}
...
wpa_driver_wext_event_wireless_custom(void *ctx, char *custom)
{
	...
	if (os_strncmp(custom, "MLME-MICHAELMICFAILURE.indication", 33) == 0) {
	...
	} else if (os_strncmp(custom, "EVENT_TDLS", 10) == 0) {
		struct wpa_driver_event_data *event_data =
			(struct wpa_driver_event_data *)custom;

		os_memcpy(data.tdls.peer, event_data->peer_mac, ETH_ALEN);
		switch (event_data->sub_index) {
		case TDLS_SETUP:
			data.tdls.oper = TDLS_REQUEST_SETUP;
			wpa_supplicant_event(ctx, EVENT_TDLS, &data);
			break;
		case TDLS_TEARDOWN:
			data.tdls.oper = TDLS_REQUEST_TEARDOWN;
			data.tdls.reason_code = WLAN_REASON_TDLS_TEARDOWN_UNSPECIFIED;
			wpa_supplicant_event(ctx, EVENT_TDLS, &data);
			break;
		case TDLS_PTI_REQ:
			data.tdls.oper = TDLS_REQUEST_PTI_REQ;
			wpa_supplicant_event(ctx, EVENT_TDLS, &data);
			break;
		default:
			break;
		}
	} else if (os_strncmp(custom, "EVENT_STA_LOW_ACK", os_strlen("EVENT_STA_LOW_ACK")) == 0) {
		struct wpa_driver_event_data *event_data =
			(struct wpa_driver_event_data *)custom;
		os_memcpy(data.tdls.peer, event_data->peer_mac, ETH_ALEN);
		switch (event_data->sub_index) {
		case TDLS_TEARDOWN:
			data.tdls.oper = TDLS_REQUEST_TEARDOWN;
			data.tdls.reason_code = WLAN_REASON_TDLS_TEARDOWN_UNREACHABLE;
			wpa_supplicant_event(ctx, EVENT_TDLS, &data);
			break;
		default:
			break;
		...
		case WDS_EXT_STA_UPDATE_EXT_INFO:
#ifdef CONFIG_TDLS
			if (event_data->ie_len > 0)
				ext_ie = (u8 *)event_data->wds_extender_ie;
			wpa_tdls_update_qhop_info(wpa_s->wpa, event_data->extender_role, ext_ie);
#endif /* CONFIG_TDLS */
			break;
		default:
			break;
	...
	while (pos + IW_EV_LCP_LEN <= end) {
		...
		case SIOCGIWAP:
			...
			if (is_zero_ether_addr(
				...
				drv->assoc_resp_ies = NULL;
#ifdef CONFIG_TDLS
				wpa_tdls_update_qhop_info(wpa_s->wpa, IEEE80211_EXTENDER_ROLE_NONE, NULL);
#endif
				    ...
				    size_t *len)
{
	...
	unsigned int max_buf_size = wpa_s->conf->scan_buf_max_size;
	struct iwreq iwr;
	u8 *res_buf;
	size_t res_buf_len;
