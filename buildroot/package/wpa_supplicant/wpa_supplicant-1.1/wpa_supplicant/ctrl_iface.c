					 ...
					 char *cmd)
{
	...
	*value++ = '\0';
	...
	} else if (os_strcasecmp(cmd, "tdls_over_qhop_enabled") == 0) {
		int enabled = atoi(value);
		wpa_printf(MSG_DEBUG, "TDLS: tdls_over_qhop_enabled=%d", enabled);
		wpa_s->wpa->tdls_over_qhop_en = enabled;
#endif /* CONFIG_TDLS */
...
static int wpa_supplicant_ctrl_iface_tdls_switch_chan(
	struct wpa_supplicant *wpa_s, char *cmd)
{
	...
	*pos++ = '\0';
	...
	if (hwaddr_aton(cmd, peer)) {
		wpa_printf(MSG_DEBUG, "CTRL_IFACE TDLS_SWITCH_CHAN:"
				" invalid address '%s'", cmd);
		return -1;
	}

	interval = atoi(pos);
	wpa_printf(MSG_DEBUG, "CTRL_IFACE TDLS_SWITCH_CHAN " MACSTR
			"interval: %u ms", MAC2STR(peer), interval);

	return wpa_tdls_start_switch_chan(wpa_s->wpa, peer, interval);
}
	...
	struct wpa_supplicant *wpa_s, char *cmd)
{
	...
	*value++ = '\0';
	...
	if ((os_strcmp(name, "psk") == 0 &&
		...
		ssid->from_wps = wpa_check_wps_ssid(wpa_s->confname, ssid->ssid);
...
static int wpa_supplicant_ctrl_iface_scan_buf_max_size(
	struct wpa_supplicant *wpa_s, char *cmd)
{
#define	WPA_SCAN_BUF_MAX_SIZE_MAX (1024 * 1024)
	int max_size = atoi(cmd);

	if ((max_size < 0) || (max_size > WPA_SCAN_BUF_MAX_SIZE_MAX))
		return -1;
...
static void wpa_supplicant_ctrl_iface_drop_sa(struct wpa_supplicant *wpa_s)
{
	...
	wpa_drv_mlme_setprotection(wpa_s, wpa_s->bssid,
				   ...
				   MLME_SETPROTECTION_KEY_TYPE_PAIRWISE, NULL);
					 ...
					 char *buf, size_t *resp_len)
{
	...
	} else if (os_strncmp(buf, WPA_CTRL_RSP, os_strlen(WPA_CTRL_RSP)) == 0)
	{
		...
		if (wpa_supplicant_ctrl_iface_tdls_teardown(wpa_s, buf + 14))
			reply_len = -1;
	} else if (os_strncmp(buf, "TDLS_SWITCH_CHAN ", 17) == 0) {
		if (wpa_supplicant_ctrl_iface_tdls_switch_chan(wpa_s, buf + 17))
			reply_len = -1;
#endif /* CONFIG_TDLS */
		...
		reply_len = hostapd_ctrl_iface_wps_timeout(wpa_s, buf + strlen("WPS_TIMEOUT "),
								reply, reply_size);
