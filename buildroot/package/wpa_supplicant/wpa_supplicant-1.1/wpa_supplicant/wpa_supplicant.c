 ...
 */
static int is_current_region_us(void)
{
	char region[16];
	FILE *fp = NULL;
	int ret = 0;
			      ...
			      u8 *wpa_ie, size_t *wpa_ie_len)
{
	...
	if (sel & WPA_CIPHER_CCMP) {
		...
		/* allow tkip for non us/fcc region */
			      ...
			      struct wpa_bss *bss, struct wpa_ssid *ssid)
{
	...
	/*
	 * Driver will be responsible to degrade to legacy mode
	 * and associate with AP with legacy rate in pure TKIP mode.
	 */
#if 0
	/* Don't associate if using TKIP and HT rates */
	if ((cipher_pairwise == CIPHER_TKIP) &&
	    (wpa_bss_get_ie(bss, WLAN_EID_HT_CAP) != NULL)) {
		wpa_printf(MSG_INFO, "Not associating, HT rates and TKIP only");
		return;
	}
#endif
...
int wpa_supplicant_setup_pairing_hash(const char *pairing_id,
...
#define WPA_SUPPLICANT_BACKUP_NAME "/mnt/jffs2/wpa_supplicant.conf.bk"
		if (wpa_s->conf == NULL) {
			...
			wpa_s->conf = wpa_config_read(confname);
			if (wpa_s->conf == NULL) {
...
struct wpa_global * wpa_supplicant_init(struct wpa_params *params)
{
	...
	global->repeater_mode = params->repeater_mode;
