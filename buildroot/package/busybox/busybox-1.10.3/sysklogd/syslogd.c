...
static void iw_sigint(int signum) {
	...
	if (iwval.iw_is_sendto_eth) {
		printf("syslog remote logging started.\n");
	} else {
		printf("syslog remote logging stopped.\n");
	}
}

static void
iw_send_pkt(char *buffer, int size) {
...
#define QDRV_NETLINK_PKTLOGGER 30
	...
	if (sock_fd < 0) {
		printf("syslog: failed to create netlink socket\n");
		return;
	}
	...
	nlh = calloc(1, NLMSG_SPACE(nsize));
	if (nlh == NULL) {
		close(sock_fd);
		return;
	}
