...
//FIXME: need to also check that last 10 bytes are zero
		...
		case INIT_SELECTING:
			...
			//DNG 3/2/2012
			//output vendor option 125 to file
			{
				uint8_t *ptr_vendor_opts, opts_len_remaining, sub_code, sub_len, ix;
				uint32_t enterprise_num;
				char opts_buf[256];
				FILE *fp;

				fp = fopen("/tmp/dhcp_option125.txt", "w");
				ptr_vendor_opts = udhcp_get_option(&packet, DHCP_VENDOR_OPTS);
				if (ptr_vendor_opts != NULL)
				{
					opts_len_remaining = ptr_vendor_opts[OPT_LEN-OPT_DATA];
					move_from_unaligned32(enterprise_num, ptr_vendor_opts);
					enterprise_num = ntohl(enterprise_num);
					//bb_error_msg("opts_len_remaining = %d\n", opts_len_remaining);
					bb_error_msg("Enterprise-number = %d\n", enterprise_num);
					if (fp)
						fprintf(fp, "Enterprise-number = %d\n", enterprise_num);
					ix = 4;
					opts_len_remaining = ptr_vendor_opts[ix];
					//bb_error_msg("opts_len_remaining = %d\n", opts_len_remaining);
					ix++;
					while (ix <= opts_len_remaining)
					{
						sub_code = ptr_vendor_opts[ix];
						sub_len = ptr_vendor_opts[ix+1];
						//bb_error_msg("sub_code = %d, sub_len = %d\n", sub_code, sub_len);
						strncpy(opts_buf, &ptr_vendor_opts[ix+2], sub_len);
						opts_buf[sub_len] = 0;
						switch(sub_code)
						{
						case 4:
							bb_error_msg("GatewayManufacturerOUI = %s\n", opts_buf);
							if (fp)
								fprintf(fp, "GatewayManufacturerOUI = %s\n", opts_buf);
							break;
						case 5:
							bb_error_msg("GatewaySerialNumber = %s\n", opts_buf);
							if (fp)
								fprintf(fp, "GatewaySerialNumber = %s\n", opts_buf);
							break;
						case 6:
							bb_error_msg("GatewayProductClass = %s\n", opts_buf);
							if (fp)
								fprintf(fp, "GatewayProductClass = %s\n", opts_buf);
							break;
						}
						ix += sub_len + 2;
						//bb_error_msg("ix = %d\n", ix);
