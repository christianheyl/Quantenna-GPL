...
static int parse_nai_realm(struct hostapd_bss_config *bss, char *buf, int line)
{
	 ...
	 * pointer even when count is zero. */
			       ...
			       char *buf, char *pos, int line)
{
...

	{
		...
		} else if (os_strcmp(buf, "radius_acct_interim_interval") == 0)
		{
			...
			errors = qtn_hapd_cfg_wpa_passphrase(bss, pos, errors, line);
		} else if (os_strcmp(buf, "wpa_psk") == 0) {
			errors = qtn_hapd_cfg_wpa_psk(bss, pos, errors, line);
...
#define PARSE_TEST_PROBABILITY(_val)					\
		...
		PARSE_TEST_PROBABILITY(corrupt_gtk_rekey_mic_probability)
			...
			if (os_strcmp(pos, "auto_lockdown") == 0)
				...
				wpa_printf(MSG_WARNING, "Line %d: unknown ap pin fail method '%s', use Default", line, pos);
			...
			if (atoi(pos) != 0) {
				...
				if (bss->wps_vendor_ext[i] == NULL) {
					wpa_printf(MSG_ERROR, "Line %d: fail to allocate WPS vendor extension buffer", line);
					return 1;
				}
