				...
				struct hostapd_radius_server *oserv)
{
	...
	if (hostapd_ip_diff(&nserv->addr,&oserv->addr) ||
				nserv->port != oserv->port) {
		return 1;
	}
			    ...
			    struct hostapd_radius_servers *conf)
{
	...
	if (radius) {
		...
		if (conf->acct_server) {
			...
			if(radius->conf &&
			   radius_server_config_diff(conf->acct_server,
		...
		if (auth_update_required) {
			if (radius_client_init_auth(radius)) {
				wpa_printf(MSG_ERROR,"Radius client auth"
		...
		if (acct_update_required) {
			if (radius_client_init_acct(radius)) {
				wpa_printf(MSG_ERROR,"Radius client acct"
						"initialization failed");
			}
		}
