...
static void wps_update_config(void *eloop_data, void *user_ctx)
{
	struct hostapd_iface *iface = eloop_data;
	char *ifn = user_ctx;
	...
	wpa_printf(MSG_DEBUG, "WPS: Reload configuration data");
	if (iface->interfaces == NULL ||
	    iface->interfaces->update_config(iface, user_ctx) < 0) {
		wpa_printf(MSG_WARNING, "WPS: Failed to update interface %s", ifn);
	}
	os_free(ifn);
}
...
enum wps_cred_write_back_state {
	WPS_CRED_SEARCH = 0,
	WPS_CRED_FOUND,
	WPS_CRED_DONE
};
...
static int hapd_wps_cred_cb(struct hostapd_data *hapd, void *ctx)
{
	 ...
	 * EAP-WSC to be finished.
	 */
	ifname = os_strdup(hapd->conf->iface);
	eloop_register_timeout(0, 1000000, wps_update_config, hapd->iface,
			       ifname);
...
static int wps_pwd_auth_fail(struct hostapd_data *hapd, void *ctx)
{
	...
	if (hapd->current_wps_lockdown == WPS_LOCKDOWN_AUTO) {
		...
		if (hapd->auto_ld.fail_count < hapd->auto_ld.max_fail_retry)
			return 0;
		wpa_printf(MSG_DEBUG, "WPS: [Auto-Lock] reach max fail count (%d), lock ap setup", hapd->auto_ld.max_fail_retry);
		     ...
		     struct hostapd_bss_config *conf)
{
	...
	os_memcpy(wps->last_wps_client_wps_type,
			...
			strlen(hapd->last_wps_client_wps_type));
...
void hostapd_deinit_wps(struct hostapd_data *hapd)
{
	...
	os_memcpy(hapd->last_wps_client_wps_type,
			...
			strlen(hapd->wps->last_wps_client_wps_type));
	os_free(hapd->wps);
	hapd->wps = NULL;
	hostapd_wps_clear_ies(hapd);
}
			      ...
			      const u8 *p2p_dev_addr)
{
	if (hapd->iface->default_pbc_bss) {
		return wps_button_pushed(hapd->iface->default_pbc_bss, (void *)p2p_dev_addr);
	}
...
{
	return wps_button_pushed(hapd, NULL);
}
