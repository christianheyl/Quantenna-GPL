				   ...
				   struct sta_info *sta, int authorized)
{
	...
	wpa_printf(MSG_DEBUG, "set sta authorized called for sta: %02x:%02x:%02x:%02x:%02x:%02x\n", sta->addr[0], sta->addr[1], sta->addr[2], sta->addr[3], sta->addr[4], sta->addr[5]);
			...
			size_t len)
{
	...
	case IEEE802_1X_TYPE_EAPOL_START:
		...
		if (eapol_start_delay && ((sta_addr = os_malloc(6)) != NULL)) {
			os_memcpy(sta_addr, sta->addr, 6);
			eloop_cancel_timeout(ieee802_1x_eapol_start_handle, hapd, sta_addr);
			eloop_register_timeout(hapd->eapol_resp_delay_s, hapd->eapol_resp_delay_us,
			...
			void *data)
{
	...
	case RADIUS_CODE_ACCESS_ACCEPT:
		...
		if (sta->vlan_id < 0
				&& sta->ssid->dynamic_vlan == DYNAMIC_VLAN_REQUIRED) {
