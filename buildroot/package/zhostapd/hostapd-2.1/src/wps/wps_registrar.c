				...
				int p2p_wildcard)
{
	...
	if (reg->pbc && reg->selected_registrar &&
	...
	if (wps->dev_pw_id == DEV_PW_PUSHBUTTON) {
		if ((wps->wps->registrar->force_pbc_overlap ||
			...
			wps_fail_event(wps->wps, WPS_M1,
				       ...
				       WPS_EI_NO_ERROR, wps->mac_addr_e, 0);
	...
	case WPS_M3:
		...
		if (ret == WPS_FAILURE || wps->state == SEND_WSC_NACK)
			wps_fail_event(wps->wps, WPS_M3, wps->config_error,
				       wps->error_indication, wps->mac_addr_e, 0);
		break;
	case WPS_M5:
						 ...
						 const struct wpabuf *msg)
{
	...
	os_memset(wps->wps->last_wps_client_wps_type, 0, sizeof(wps->wps->last_wps_client_wps_type));
	if (wps->pbc) {
