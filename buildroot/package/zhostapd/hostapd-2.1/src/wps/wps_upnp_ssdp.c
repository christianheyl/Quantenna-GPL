						...
						void *user_ctx)
{
	...
	if (get_netif_info(sm->net_if, &ip_addr, &sm->ip_addr_text,
			sm->mac_addr)) {
		wpa_printf(MSG_ERROR, "WPS UPnP: Could not get IP/MAC address "
			   "for %s. Does it have IP address?", sm->net_if);
	}

	if (ip_addr > 0 && ip_addr != sm->ip_addr) {
		wpa_printf(MSG_DEBUG, "WPS UPnP: IP address changed to: %s "
			   "for %s.", sm->ip_addr_text, sm->net_if);
