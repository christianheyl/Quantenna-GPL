...
static int hostapd_config_parse_cipher(int line, const char *value)
{
	...
	while (*start != '\0') {
		...
		*end = '\0';
		...
		if ((os_strcmp(start, "CCMP") == 0) || (os_strcmp(start, "TKIP") == 0))
			val |= WPA_CIPHER_CCMP;
#else
				    ...
				    struct hostapd_config *conf)
{
...
#ifdef CONFIG_WPS2
	if (bss->wps_state && bss->macaddr_acl == DENY_UNLESS_ACCEPTED &&
	...
	if (bss->wps_state && bss->ssid.wep.keys_set && bss->wpa == 0) {
		wpa_printf(MSG_WARNING, "WPS: WEP configuration forced WPS to be "
			   "disabled");
		bss->wps_state = 0;
	}
#endif /* CONFIG_WPS2 */

	if (bss->allow_pbc_overlap == WPS_ALLOW_PBC_OVERLAP) {
		wpa_printf(MSG_WARNING, "WPS: PBC overlap is allowed.");
		wpabuf_free(bss->wps_quantenna_ext);
		bss->wps_quantenna_ext = NULL;

		bss->wps_quantenna_ext = wpabuf_alloc(WPS_QUANTENNA_EXT_ATTR_SIZE);
		if (!(bss->wps_quantenna_ext)) {
			wpa_printf(MSG_ERROR, "WPS: wps_quantenna_ext buffer fails to "
				"allocate");
			return -1;
		}

		wpabuf_put_be24(bss->wps_quantenna_ext, WPS_VENDOR_ID_QTN);
		wpabuf_put_u8(bss->wps_quantenna_ext, WPS_ALLOW_PBC_VL_SUB_ID);
		wpabuf_put_u8(bss->wps_quantenna_ext, 1);
		wpabuf_put_u8(bss->wps_quantenna_ext, WPS_ALLOW_PBC_OVERLAP);
	} else if (bss->allow_pbc_overlap == WPS_FORBID_PBC_OVERLAP) {
		wpa_printf(MSG_WARNING, "WPS: PBC overlap is forbidden as spec says.");
		wpabuf_free(bss->wps_quantenna_ext);
		bss->wps_quantenna_ext = NULL;
	}
...
struct hostapd_config * hostapd_config_read(const char *fname)
{
		...
		*pos = '\0';
		...
		} else if (os_strcmp(buf, "radius_acct_interim_interval") == 0)
		{
			...
			if (len < 8 || len > 63) {
				...
				os_free(bss->ssid.wpa_passphrase_md5);

				bss->ssid.wpa_passphrase = os_strdup(pos);
				memset(wpa_passphrase_md5, 0, sizeof(wpa_passphrase_md5));
				local_md5_convert_passphrase(bss->ssid.wpa_passphrase, wpa_passphrase_md5);
				bss->ssid.wpa_passphrase_md5 = os_strdup(wpa_passphrase_md5);
			}
		} else if (os_strcmp(buf, "wpa_psk") == 0) {
			char wpa_psk_md5[64] = {0};
			char temp[65] = {0};
			...
			if (bss->ssid.wpa_psk == NULL)
				errors++;
					...
					hexstr2bin(wpa_psk_md5, bss->ssid.wpa_psk->psk_md5, PMK_LEN) ||
					pos[PMK_LEN * 2] != '\0') {
		...
		} else if (os_strcmp(buf, "assoc_sa_query_retry_timeout") == 0)
		{
			...
			if (atoi(pos) >= 0 && atoi(pos) <= 2) {
				...
				wpa_printf(MSG_ERROR, "Line %d: pairing_enable"
						" not right", line);
				errors++;
			}
			...
			if (atoi(pos) == 0) {
				bss->allow_pbc_overlap = WPS_FORBID_PBC_OVERLAP;
			} else {
				bss->allow_pbc_overlap = WPS_ALLOW_PBC_OVERLAP;
			}
			...
			bss->ap_pin_fail_method = os_strdup(pos);
		} else if (os_strcmp(buf, "auto_lockdown_max_retry") == 0) {
			bss->auto_lockdown_max_retry = atoi(pos);
	...
	if (unknown_fields)
		wpa_printf(MSG_WARNING, "%d unknown fields found, skip it", unknown_fields);
