 ...
 * @pp_enable: enable or disable pair protection
 */
...
struct wps_registrar_config {
	...
	/**
	 * ignore_broadcast_ssid - Wheter the ssid is hidden
	 */
...
enum wps_event {
	/**
	 * WPS_EV_INIT - WPS finish initilization
	 */
	WPS_EV_INIT,
...
struct wps_context {
	...
	/**
	 * wps_state - Current (internal) WPS state
	 */
	...
	/**
	 * probing_db_cb - process probing database
	 * @ctx: Higher layer context data (cb_ctx)
	 * @addr: MAC address to update
	 * @remove: 1 remove from db, otherwise add to db
	 */
	int (*probing_db_cb)(struct hostapd_data *hapd, const u8 *addr, int remove);
	...
	/**
	 * wps_stas - record current STAs showing WPS on probe req
	 */
#define WPS_MAX_STAS_RECORD_SIZE	8
	struct {
		u8 stas[WPS_MAX_STAS_RECORD_SIZE][ETH_ALEN];
		int used;
	} wps_stas;
