...
int wps_build_quantenna_ext(struct wps_device_data *dev, struct wpabuf *msg)
{
	if (dev->quantenna_ext == NULL)
		return 0;

	wpa_hexdump(MSG_DEBUG, "WPS:  * Vendor Extension",
		    wpabuf_head_u8(dev->quantenna_ext),
		    wpabuf_len(dev->quantenna_ext));
	wpabuf_put_be16(msg, ATTR_VENDOR_EXT);
	wpabuf_put_be16(msg, wpabuf_len(dev->quantenna_ext));
	wpabuf_put_buf(msg, dev->quantenna_ext);
			 ...
			 const struct wps_device_data *src)
{
	...
	if (src->pp_device_name)
		dst->pp_device_name = os_strdup(src->pp_device_name);
	if (src->pp_device_name_bl)
		dst->pp_device_name_bl = os_strdup(src->pp_device_name_bl);
