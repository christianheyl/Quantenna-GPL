					  ...
					  const u8 *addr, const u8 *uuid_e)
{
	...
	while (pbc) {
		if (now.sec > pbc->timestamp.sec + qtn_wps_pbc_timeout) {
			prev->next = NULL;
...
static void wps_registrar_stop_pbc(struct wps_registrar *reg)
{
	...
	if (reg->wps_on_hidden_ssid && reg->ignore_broadcast_ssid) {
		wpa_printf(MSG_DEBUG, "WPS: wps_on_hidden_ssid && ignore_broadcast_ssid,"
				"hide SSID\n");
				...
				const u8 *p2p_dev_addr)
{
	...
	eloop_cancel_timeout(wps_registrar_set_selected_timeout, reg, NULL);
	eloop_cancel_timeout(wps_registrar_pbc_timeout, reg, NULL);
	eloop_register_timeout(qtn_wps_pbc_timeout, 0, wps_registrar_pbc_timeout,
			       reg, NULL);
	if ((reg->wps) && (reg->wps->event_cb)) {
				...
				int p2p_wildcard)
{
	...
	if (reg->wps != NULL && reg->wps->vendor == WPS_VENDOR_NETGEAR) {
		if (reg->selected_registrar &&
			ap_get_sta(reg->cb_ctx, addr) == NULL) {
			reg->wps->probing_db_cb(reg->cb_ctx, addr, 0);
		}
	}
...
static int wps_set_ie(struct wps_registrar *reg)
{
	...
	if (wps_build_version(beacon) ||
	    ...
	    wps_build_quantenna_ext(&reg->wps->dev, beacon)) {
		wpabuf_free(beacon);
		wpabuf_free(probe);
		return -1;
	}
	...
	if (wps_build_version(probe) ||
	    ...
	    wps_build_quantenna_ext(&reg->wps->dev, probe)) {
		wpabuf_free(beacon);
		wpabuf_free(probe);
		return -1;
	}
...
#define NTGR_PASSWD_SIZE_BASE		8
	...
	os_memcpy(wps->cred.ssid, ssid, strlen(ssid) + 1);
	wps->cred.ssid_len = strlen(ssid);
...
static struct wpabuf * wps_build_m2(struct wps_data *wps)
{
	...
	if (wps_build_version(msg) ||
	    ...
	    wps_derive_keys(wps) ||
	    wps_build_auth_type_flags(wps, msg, 0) ||
	    wps_build_encr_type_flags(wps, msg, 0) ||
...
#define STB_BCM_DEVNAME	"Cisco Client"
	...
	if (!wps || !wps->wps ||
		!attr || !attr->manufacturer || !attr->dev_name)
	...
	if (wps->wps->auth_types == (WPS_AUTH_WPA2PSK | WPS_AUTH_WPAPSK) &&
			...
			strncmp(attr->manufacturer, STB_BCM_MANUFACTURER, attr->manufacturer_len) == 0 &&
			strncmp(attr->dev_name, STB_BCM_DEVNAME, attr->dev_name_len) == 0) {
...
static int wps_process_encr_type_flags(struct wps_data *wps, struct wps_parse_attr *attr)
{
	u16 encr_types;
...
static int wps_verify_device_name(const struct wps_context *wps,
				  const u8 * dev_name,
					   ...
					   struct wps_parse_attr *attr)
{
	...
	if (wps_verify_device_name(wps->wps, attr->dev_name, attr->dev_name_len) == WPS_PP_NAME_CHK_REJECT)
		return WPS_FAILURE;
						 ...
						 const struct wpabuf *msg)
{
	...
	if (wps->wps->wps_state == WPS_STATE_NOT_CONFIGURED && wps->new_psk &&
		...
		if (wps->wps->vendor == WPS_VENDOR_NETGEAR) {
			...
			/* we generate mix mode in our setting */
	...
	if (wps->pbc) {
		...
		os_memcpy(wps->wps->last_wps_client_wps_type, "pin", strlen("pin"));
		wps_registrar_pin_completed(wps->wps->registrar);
	}
