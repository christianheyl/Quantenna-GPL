...
int wps_build_auth_type_flags(struct wps_data *wps, struct wpabuf *msg, int enrollee)
{
	u16 auth_types = WPS_AUTH_TYPES_Q;
#ifdef CONFIG_WPS2
	if (enrollee != 0) {
		auth_types &= ~WPS_AUTH_SHARED;
	} else {
		auth_types = wps->wps->auth_types | WPS_AUTH_OPEN;
	}
#endif /* CONFIG_WPS2 */
...
int wps_build_encr_type_flags(struct wps_data *wps, struct wpabuf *msg, int enrollee)
{
	u16 encr_types = WPS_ENCR_TYPES_Q;
#ifdef CONFIG_WPS2
	if (enrollee != 0) {
		encr_types &= ~WPS_ENCR_WEP;
	} else {
		/*
		 * WPA/TKIP is not supported in WPS2.0, but it's used in WPS1.0 devices.
		 * So add it here for backward complicance in WPS handshaking.
		 */
		encr_types = wps->wps->encr_types | WPS_ENCR_NONE | WPS_ENCR_TKIP;
	}
#endif /* CONFIG_WPS2 */
