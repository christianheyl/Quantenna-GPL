...
struct ap_pin_fail_dfl_data {
	unsigned int accu_fail;
	unsigned int consecutive_fail;
	unsigned int lockout_time;
};

struct ap_pin_fail_auto_lockdown_data {
	unsigned int accu_fail;
	unsigned int fail_threshold;
#define AP_PIN_FAIL_THRESHOLD_DEFAULT	3
};
...
#define AID_WORDS ((2008 + 31) / 32)
	...
	void *ap_pin_fail_m;
	struct {
		struct ap_pin_fail_auto_lockdown_data auto_lockdown;
		struct ap_pin_fail_dfl_data dfl;
	} ap_pin_fail_data;
...
struct hostapd_iface {
	...
	int (*update_bss_config)(struct hostapd_iface *iface, const char *bss);
	...
	struct hostapd_data *default_bss_for_pbc;
};
...
int hostapd_create_bss_config(struct hostapd_iface *iface, const char *bss);
struct hostapd_data *
