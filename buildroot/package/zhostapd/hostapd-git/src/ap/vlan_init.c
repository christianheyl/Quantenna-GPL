			    ...
			    struct hostapd_vlan *vlan)
{
	while (vlan) {
		if (vlan->vlan_id != VLAN_ID_WILDCARD) {
			if (hostapd_vlan_group_add(hapd, hapd->conf->iface, vlan->vlan_id)) {
				if (errno != EEXIST) {
...
static void vlan_reset_wpa_group(struct hostapd_data *hapd)
{
	...
	/*
	* For each VLAN group, mark first_sta_seen FALSE so
	* GTK is re-calculated and distributed when it's
	* created again
	*/
	if (hapd->wpa_auth) {
				...
				struct hostapd_vlan *vlan)
{
	...
	while (vlan) {
		...
		if (vlan->vlan_id != VLAN_ID_WILDCARD &&
		    hostapd_vlan_group_remove(hapd, hapd->conf->iface, vlan->vlan_id)) {
				       ...
				       int vlan_id)
{
	...
	*pos++ = '\0';
	...
	if (hostapd_vlan_group_add(hapd, hapd->conf->iface, vlan_id)) {
		os_free(n);
		return NULL;
	}
...
int vlan_remove_dynamic(struct hostapd_data *hapd, int vlan_id)
{
	...
	if (vlan->dynamic_vlan == 0)
		hostapd_vlan_group_remove(hapd, hapd->conf->iface, vlan_id);
