...
struct iwevent_net_s {
	...
	char iw_eth_buffer[ETHER_MAX_LEN];
	char iw_syslog_buffer[ETHER_MAX_LEN];
	long iw_total_sent_packets;
	int  iw_is_sendto_eth;
	   ...
	   char *	name)
{
  ...
  /* Get interface name */
  /* For some reason, ifi_index in the RTNETLINK message gets bit 17 set.
   * Don't have time to track down why.
   */
  irq.ifr_ifindex = ifindex & 0xFF;
...
{
	va_list ap;
	char loc_buf[IW_PRINTBUF_SZ] = { 0 };

	va_start(ap, fmt);
	vsnprintf(loc_buf, sizeof(loc_buf), fmt, ap);
	va_end(ap);

	if (iwval.iw_is_sendto_eth == 0 && iwval.iw_is_sendto_syslog == 0) {
		printf("%s", loc_buf);
	} else {
		strncat(iwval.iw_eth_buffer + ETHER_HDR_LEN, loc_buf, sizeof(loc_buf) - 1);
		strncat(iwval.iw_syslog_buffer, loc_buf, sizeof(loc_buf) - 1);
	}
}
...
static void iw_sigint(int signum) {

  /* 
   * interface should already be in promisc
   * mode, anyways don't touch the flags 
   */
  if (iwval.iw_total_sent_packets) {
      printf("iwevent sent: %ld packets\n", iwval.iw_total_sent_packets);
  }
  exit(0);
}

static void iw_send_pkt(void) {
...
#define QDRV_NETLINK_PKTLOGGER 30
	...
	memcpy(NLMSG_DATA(nlh), iwval.iw_eth_buffer + ETHER_HDR_LEN,
			strlen(iwval.iw_eth_buffer + ETHER_HDR_LEN));
	nlh->nlmsg_pid = getpid();
	nlh->nlmsg_len = strlen(iwval.iw_eth_buffer + ETHER_HDR_LEN);
#define QDRV_NETDEBUG_TYPE_IWEVENT 5
	nlh->nlmsg_type = QDRV_NETDEBUG_TYPE_IWEVENT;
	iov.iov_base = (void *)nlh;
	iov.iov_len = NLMSG_SPACE(strlen(iwval.iw_eth_buffer + ETHER_HDR_LEN));
...
{
	char* msg = iwval.iw_syslog_buffer;

	msg[strlen(msg)] = '\n';
	syslog(LOG_NOTICE, msg);
	memset(iwval.iw_syslog_buffer, 0, sizeof(iwval.iw_syslog_buffer));
}
		   ...
		   int		len)
{
  ...
  do
      ...
      if(ret != 0)
	{
	  ...
	  if (iwval.iw_is_sendto_eth) {
	      iw_send_pkt();
	      memset(iwval.iw_eth_buffer, 0, sizeof(iwval.iw_eth_buffer));
	  }
	}
    }
  while(ret > 0);
...
iw_usage(int status)
{
  fputs("Usage: iwevent [OPTIONS]\n"
	...
	"     -i,--interface Send output to the Ethernet interface.\n"
     ...
     char *	argv[])
{
  ...
  while((opt = getopt_long(argc, argv, "hvsi:", long_opts_iwevent, NULL)) > 0)
    {
      switch(opt)
	{
	case 'h':
	...
	case 'i':
	  iwval.iw_is_sendto_eth = 1;
	  strncpy(iwval.iw_ifname, optarg, sizeof(iwval.iw_ifname) - 1);
	  iwval.iw_ifname[sizeof(iwval.iw_ifname) - 1] = '\0';
	  break;
